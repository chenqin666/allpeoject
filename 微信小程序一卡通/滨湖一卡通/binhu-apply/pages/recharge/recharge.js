 ...
 /*支付宝支付*/
  confirmpsd: function (){
    ...
    if (this.data.moneys == '') {
      ...
      var appid = 'wxbc5fe8641a32ef8f';
      http.request(api.payment, data, wx.getStorageSync("token"), appid,'').then(function (result) {
        if (result.code == 0) {
  ...
  card_lists: function (type) {
    ...
    var appid = 'wxbc5fe8641a32ef8f';
    http.request(api.feepercent, data, wx.getStorageSync("token"), appid, '', true).then(function (result) {
      if (result.code == 0) {
        // 授权成功并且服务器端登录成功
